...
def build_knn_graphs_for_mdata(mdata, k=15, metric="euclidean"):
    """Build KNN graphs for both RNA and ATAC embeddings"""
    print(f"Building KNN graphs with k={k}, metric={metric}")
    
    # Get embeddings
    Z_rna = mdata["rna"].obsm["X_pca_rna"]
    Z_atac = mdata["atac"].obsm["X_pca_atac"]
    
    # Build KNN graphs
    D_rna, I_rna, self_rna = build_knn_gpu(Z_rna, k=k, metric=metric)
    store_knn_results(mdata["rna"], "rna", D_rna, I_rna, k, metric, self_rna)
    
    D_atac, I_atac, self_atac = build_knn_gpu(Z_atac, k=k, metric=metric)
    store_knn_results(mdata["atac"], "atac", D_atac, I_atac, k, metric, self_atac)
    
    # Store summary
    mdata.uns["knn_summary"] = {
        "k": k,
        "metrics": {"rna": metric, "atac": metric},
        "includes_self": {"rna": self_rna, "atac": self_atac}
    }
    
    print(f"[KNN] Complete - RNA and ATAC graphs built")
    return (D_rna, I_rna, self_rna), (D_atac, I_atac, self_atac)
